...
work=$(mktemp -d "$root/.retrom-build/retrom-potator-web.XXXXXX")
...
if ! docker run --rm --platform linux/amd64 --hostname retrom-potator \
  ...
  /recipe/build-emulatorjs-core.sh potator \
...
stage="$work/stage"
mkdir -p "$stage"
install -m 0644 "$work/raw/potator_libretro.js" "$stage/"
install -m 0644 "$work/raw/potator_libretro.wasm" "$stage/"
cat "$root/LICENSE" "$work/raw/retroarch-COPYING" > "$stage/license.txt"
printf '%s\n' '{"minimumEJSVersion":"4.2.2","version":"1.18"}' > "$stage/build.json"
printf '%s\n' '{"name":"potator","extensions":["bin","sv"],"makeoptions":{"buildpath":"./platform/libretro","makescript":"Makefile","arguments":[]},"options":{},"save":true,"license":"LICENSE","repo":"https://github.com/retrom-project/potator"}' > "$stage/core.json"
chmod 0644 "$stage/build.json" "$stage/core.json" "$stage/license.txt"

(cd "$stage" && 7z a -mtm=off -mta=off -mtc=off -bd -bso0 -bsp0 -t7z "$output/potator-wasm.data" \
  potator_libretro.js potator_libretro.wasm build.json core.json license.txt)
