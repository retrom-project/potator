...
emmake make -C platform/libretro -f Makefile clean platform=emscripten "$@"
emmake make -C platform/libretro -j"4" -f Makefile platform=emscripten \
  INITIAL_HEAP=268435456 AUTO_MEMORY_GROWTH=1 "$@"

archive=$(find platform/libretro -maxdepth 1 -type f -name "${core_name}_libretro_emscripten.bc" -print)
